...
const char* password = "apa12345"; // Enter WiFi password
...
//    if (client.connect("ESP32Client", mqttUser, mqttPassword )) {
    ...
    client.subscribe("autoSetSubai");
    client.subscribe("SetPointSubai");   
    client.subscribe("Nilai_KcSubai");
    client.subscribe("Nilai_tauISubai");
    client.subscribe("Nilai_tauDSubai");
  }
}
...
void loop() { 
  ...
  if(Topic=="autoSetSubai"){
    autoSet=Payload.toInt();
  }
   if(Topic=="Nilai_KcSubai"){
    Kc=Payload.toFloat();
  }
   if(Topic=="Nilai_tauISubai"){
    tauI=Payload.toFloat();
  }
   if(Topic=="Nilai_tauDSubai"){
    tauD=Payload.toFloat()/6;
  }
   if(Topic=="SetPointSubai"){
    sp=Payload.toFloat();
  }
  Serial.println("<-------------------------->");
  ...
  client.publish("Suhu1Subai",suhu1);

  dtostrf(cel1, 1, 0, suhu2);
  client.publish("Suhu2Subai",suhu2);

  dtostrf(sp, 1, 0, Tampil_SP);
  client.publish("Tampil_SPSubai",Tampil_SP);

  dtostrf(Kc, 1, 0, Tampil_Kc);  
  client.publish("Tampil_KcSubai",Tampil_Kc);

  dtostrf(tauI, 1, 0, Tampil_tauI);
  client.publish("Tampil_tauISubai",Tampil_tauI);

  dtostrf(tauD, 1, 0, Tampil_tauD);
  client.publish("Tampil_tauDSubai",Tampil_tauD);  
  ...
  if(autoSet==1){
    ...
    // bisa diubah2, atau sesuai yg muncul terakhir
     ...
     client.publish("Nilai_opSubai",Nilai_op);
  }
}
