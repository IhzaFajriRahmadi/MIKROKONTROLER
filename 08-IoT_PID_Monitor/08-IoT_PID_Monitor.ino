...
void loop() { 
  ...
  client.publish("Suhu1Raihan",suhu1);

  dtostrf(sp, 1, 0, SetPoint);
  client.publish("SetPointRaihan",SetPoint);

  dtostrf(op, 1, 0, Nilai_op);
  client.publish("Nilai_opRaihan",Nilai_op);

  delay (200);

  dtostrf(cel1, 1, 0, suhu2);
  client.publish("Suhu2Raihan",suhu2);
